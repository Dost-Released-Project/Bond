...
public partial class BaseCharacter
{
    ...
    public async UniTask<bool> TryRunSelfTurnAnomalyAsync()
    {
        ...
        {
            // 평상시 — 계획된 행동을 그대로 수행(스윙 미발동, 오버라이드 안 함).
            Debug.Log($"<color=grey>[자기턴 판정]</color> {Name} — 평상시 행동 유지(스윙 미발동).");
            return false;
        }
        ...
        Debug.Log($"<color=magenta>[자기턴 스윙:{result}]</color> {Name} 의 성향이 계획 행동을 대체합니다. ({eff?.Description})");
        if (result == ReactionResult.Anomaly) MarkAnomaly(); // 특이(돌발)만 아군 관찰 플래그 — 강화(각성)는 제외

        if (eff != null)
            await eff.Apply(this, execution, ctx);

        await UniTask.Delay(1000); // 스윙 연출 마무리
        return true;
    }
    ...
    /// 굴림에 실패(=스윙 안 함)하면 평상시 행동(ReactionResult.Success).<br/>
    ...
    /// <summary>
    /// 역할 리액션의 특이(돌발) 행동 확률 = clamp(기본 + 스트레스 − 지능 − 관계, 최저 5%, 1).
    /// 관계가 낮을수록·스트레스가 높을수록·지능이 낮을수록 높아진다.
    ...
    /// <summary>
    /// 성향 리액션의 강화(유대적 각성) 행동 확률 = clamp(기본 + 관계 + 스트레스 + 지능, 최저 5%, 1).
    /// 관계가 높을수록·스트레스 임계일수록·지능이 높을수록(통제→각성 전환) 높아진다(GDD §4).
    /// 상수는 밸런스 튜닝 대상.
    /// </summary>
    ...
    /// <summary>
    /// 판정에 쓰는 관계 수치. 리액터↔관찰 대상(subjects, 자신 제외)의 평균.
