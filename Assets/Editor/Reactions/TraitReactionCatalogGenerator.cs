    ...
    public static class TraitReactionCatalogGenerator
    {
        ...
        public static void Generate()
        {
            ...
            Add("TRT_004", "의심 많은", E_TraitType.Negative,
                Def("TRT_004_RDEF", RoleType.None).Name("의심 많은 — 단독 행동", "아군 공격 빗나감 → 해당 라운드 전체 봉인"
                    )
            ...
            Add("TRT_005", "냉혹한", E_TraitType.Negative,
                Def("TRT_005_RDEF", RoleType.None).Name("냉혹한 — 방치", "아군 HP30%↓ → 그 아군 설계 리액션 봉인 + 그 아군 스트레스+15"
                    )
                    .Phase(E_ReactionPhase.PostApply).Observe(E_ObserveFilter.OtherAlly)
            ...
            // 006 허세: 아군 치명타 → 그 대상에 마무리 + 연쇄 리셋
            Add("TRT_006", "허세 부리는", E_TraitType.Neutral,
                Def("TRT_006_RDEF", RoleType.None).Name("허세 — 끼어들기", "아군 치명타 → 그 대상에 마무리 + 연쇄 리셋"
                    )
                    .Phase(E_ReactionPhase.PostApply).Observe(E_ObserveFilter.OtherAlly)
            ...
            Add("TRT_008", "복수심 강한", E_TraitType.Neutral,
                Def("TRT_008_RDEF", RoleType.None).Name("복수심 — 충동 반격", "본인 피격 → 공격자 반격 + 역할 리액션 봉인"
                    )
                    .Phase(E_ReactionPhase.PostApply).Observe(E_ObserveFilter.Self)
            ...
            Add("TRT_009", "인정욕구 강한", E_TraitType.Neutral,
                Def("TRT_009_RDEF", RoleType.None).Name("인정욕구 — 무모한 돌격", "파티 평균 스트레스 60%↑ → 전열 돌파"
                    )
                    .Phase(E_ReactionPhase.OnSelfTurn).Observe(E_ObserveFilter.Self)
            ...
            Add("TRT_013", "쉽게 지치는", E_TraitType.Negative,
                Def("TRT_013_RDEF", RoleType.None).Name("쉽게 지치는 — 강제 휴식", "연속 3회 리액션 → 탈진(행동 생략) + 전체 봉인"
                    )
                    .Phase(E_ReactionPhase.OnSelfTurn).Observe(E_ObserveFilter.Self)
                    .When(ReactionCountAtLeast(3))
                    .Do(Seal(SealKind.All, turns: 1)));

            // 014 과시욕: 본인 치명타 → 남은 리액션 봉인
            Add("TRT_014", "과시욕 있는", E_TraitType.Neutral,
                Def("TRT_014_RDEF", RoleType.None).Name("과시욕 — 독무대", "본인 치명타 → 남은 리액션 봉인"
                    )
                    .Phase(E_ReactionPhase.PostApply).Observe(E_ObserveFilter.Self)
                    .When(SubjectIs(E_TargetFilter.Caster), Crit())
                    .Do(Seal(SealKind.All, turns: 1)));
            ...
            Add("TRT_016", "규율적인", E_TraitType.Positive,
                Def("TRT_016_RDEF", RoleType.None).Name("규율적 — 자기 통제", "아군 돌발 후 내 턴 → 자기통제(리액션 1개 봉인)"
                    )
                    .Phase(E_ReactionPhase.OnSelfTurn).Observe(E_ObserveFilter.Self)
                    .When(AllyAnomaly())
            ...
            Add("TRT_018", "완벽주의적", E_TraitType.Negative,
                Def("TRT_018_RDEF", RoleType.None).Name("완벽주의 — 자기 처벌", "본인 공격 빗나감 → 다음 턴 1개 봉인 + 스트레스+10"
                    )
                    .Phase(E_ReactionPhase.PostApply).Observe(E_ObserveFilter.Self)
                    .When(SubjectIs(E_TargetFilter.Caster), Evaded())
            ...
            Add("TRT_019", "무모한", E_TraitType.Neutral,
                Def("TRT_019_RDEF", RoleType.None).Name("무모한 — 과신한 돌진", "자기 턴 HP70%↑ → 전열 + 가장 먼 적 단독 돌진"
                    )
                    .Phase(E_ReactionPhase.OnSelfTurn).Observe(E_ObserveFilter.Self)
                    .When(HpAbove(0.7f))
